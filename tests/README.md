...
milliseconds; safe to wire into CI on every push.
...
unit test is. Treat Tier 2 as a **pre-release gate** (run the scenario whose
bug class is closest to what you just changed, or all of them before cutting
a version) rather than a per-commit CI check. Tier 1 is cheap enough to run
on every push; Tier 2 is not.
