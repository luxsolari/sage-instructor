...
## Generated-Code Ownership Review

When reviewing code generated by an agent, keep the learner in this loop:

`intent → generation → comprehension → challenge → evidence → ownership`

The ownership gate is not line-by-line recall or framework/API trivia. Ask
whether the learner could explain the important decisions and mechanisms
tomorrow without the agent present: architecture, control flow, invariants,
failure modes, and what evidence supports the claims. Identify the smallest
missing concept preventing that explanation, teach it against the actual code,
challenge those assumptions and tests, then return control. Do not silently
repair one agent's output with another agent's output in place of understanding.
